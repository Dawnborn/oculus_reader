...
from HandHandlerP2 import modes, hand_fully_open, cube_hand_close
...
def main():
    ...
    # ip_addr = IP_ADDRESS_LEFT

    hand = pyHandApi.HandApi(ip_addr)
    time.sleep(1)
    flag = True
    try:
        while(flag):
    ...
    hand.setJoints(joints=hand_fully_open,velocity=20, acceleration=20)
    time.sleep(0.5)
    hand_close=False
    hand_close_pre=False
    ...
    joints_pos_home = np.array([ 0.49691393, -0.07177353, -1.04238794, -2.69007516,  2.53164369,  0.88213488, 0.]) # 右侧p2右arm

    joints_pos_home = np.array([-0.44972005,  0.63570325,  1.46048162,  2.01833534,  1.5006887,   0.68628866,  0.])
    ...
    try:
        while(True):
            ...
            elif ip_addr==IP_ADDRESS_LEFT:
                leftGrip = buttons['leftGrip'][0]
                leftTrig = buttons['leftTrig'][0]
            ...
            if leftTrig>0.5:
                # print("left trigger: {}".format(leftTrig))
                if not hand_close:
                    print("close hand")
                    hand.setJoints(joints=cube_hand_close,velocity=20, acceleration=20)
                    time.sleep(0.5)
                    hand_close = True
            else:
                # print("left trigger: {} hand close {}".format(leftTrig, hand_close))
                if hand_close:
                    print("open hand")
                    hand.setJoints(joints=hand_fully_open,velocity=20, acceleration=20)
                    time.sleep(0.5)
            ...
            if(leftGrip>0.5):
                ...
                if dot_translation_pre is None:
                    ...
                    R_tcp=np.array([[-0.2741742,   0.93061001,  0.24247521],
                        [ 0.95942026,  0.28196798,  0.0026652 ],
                        [-0.06588973,  0.2333658,  -0.97015393]])
                    R_tcp = np.array([[-0.82277966, -0.27490439, -0.4974553 ],
                                        [ 0.40584716, -0.89691145, -0.17560953],
                                        [-0.39789663, -0.34637854,  0.84952893]])
                    
                    R_target = R_bot_dot@R_tcp
                    orientation = mat2rxryrz(R_target)

                    R_tcp_current = rxryrz2mat(tcp_current_pose[3:6])
                    R_trans = (R_target.T)@R_tcp_current
                    print("R_trans: {}".format(R_trans))


                    orientation_pre = tcp_current_pose[3:6]
                    translation_pre = tcp_current_pose[:3]

                    # tcp_pos_target = np.concatenate([translation_pre, orientation_pre],axis=-1)
                    # tcp_pos_target = np.concatenate([translation_pre,orientation],axis=-1)
                    # tcp_pos_target = np.concatenate([tcp_next_translation,orientation_pre],axis=-1)
