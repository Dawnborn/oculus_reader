    ...
    while(True):
        ...
        if(rightGrip>0.5):
            ...
            if dot_translation_pre is None:
                ...
                orientation_diff = orientation - orientation_pre
                print("orientation_diff: {}".format(orientation_diff))

                tcp_pos_target = np.concatenate([tcp_next_translation, orientation_pre+orient_scale*orientation_diff],axis=-1)
                ...
                #     wait=wait
                # )
                for ii in range(BOT_TIME_SCALE):
                    success = diana.move_tcp_servoL(tcp_pos=tcp_pos_target, t=TIME_STEP/BOT_TIME_SCALE, gain=600, realiable=True)
